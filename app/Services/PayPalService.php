...
class PayPalService
{
    ...
    public function __construct()
    {
    	 ...
    	 // dd($this->config);
        if ($this->config['paypal_client_id'] == '' || $this->config['paypal_secret_id'] == '') {
    ...
    public function processPayment($order)
    {
        ...
        foreach ($order->items as $item)
        {
            ...
            $orderItems[$item->id]->setName($item->product->name)
                ->setCurrency($this->config['currency_code'])
        ...
        $amount = new Amount();
        $amount->setCurrency($this->config['currency_code'])
                ->setTotal(sprintf('%0.2f', $order->subtotal))
                ->setDetails($details);
